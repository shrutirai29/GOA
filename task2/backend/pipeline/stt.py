...
# Sarvam language code → our format
_LANG_MAP = {
...
def _ensure_wav(audio: bytes) -> bytes:
    ...
    Sarvam only supports WAV format.
    """
    # Already WAV
    if audio[:4] == b"RIFF":
        return audio
...
class SttProvider:
    ...
    # ------------------------------------------------------------- providers
    def _sarvam(self, audio: bytes) -> tuple[str, str]:
        import httpx

        # Convert browser formats (webm/mp4/ogg) to WAV — Sarvam only supports WAV
        wav_audio = _ensure_wav(audio)
        ...
        # Detect language from Sarvam response
        raw_lang = body.get("language_code", "")
        detected_lang = _LANG_MAP.get(raw_lang, raw_lang.split("-")[0] if raw_lang else "")
        return text, detected_lang

    def _elevenlabs(self, audio: bytes) -> tuple[str, str]:
        import httpx
